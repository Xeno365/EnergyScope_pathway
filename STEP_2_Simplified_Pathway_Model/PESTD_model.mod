...
#NEW DEPENDENT:
set AGE {TECHNOLOGIES,PHASE} within PHASE union {"2010_2020"} union {"STILL_IN_USE"};
...
param decom_allowed {PHASE,PHASE union {"2010_2020"},TECHNOLOGIES} default 0;
...
param limit_ren_changes >= 0;
param efficiency {YEARS} >=0 default 1;
param years_done {TECHNOLOGIES,PHASE};
...
var F_new {PHASE union {"2010_2020"}, TECHNOLOGIES} >= 0; #[GW/GWh] Accounts for the additional new capacity installed in a new phase
var F_decom {PHASE,PHASE union {"2010_2020"}, TECHNOLOGIES} >= 0; #[GW] Accounts for the decommissioned capacity in a new phase
...
var C_inv_return {TECHNOLOGIES,PHASE} >=0;
#var TrueTransitionCost >=0;
...
subject to phase_new_build {p in PHASE, y_start in PHASE_START[p], y_stop in PHASE_STOP[p], i in TECHNOLOGIES diff {"EFFICIENCY"}}:
	F[y_stop,i] = F[y_start,i] + F_new[p,i] - F_old [p,i] - sum {p2 in {PHASE union {"2010_2020"}}} F_decom[p,p2,i];


# [Eq. XX] Impose decom_allowed to 0 when not physical
subject to define_f_decom_properly {p_decom in PHASE, p_built in PHASE union {"2010_2020"}, i in TECHNOLOGIES}:
	if decom_allowed[p_decom,p_built,i] == 0 then F_decom [p_decom,p_built,i] = 0;


# [Eq. XX] Intialise the first phase based on YEAR_2020 results
subject to F_new_initiatlisation {tech in TECHNOLOGIES}:
	F_new ["2010_2020",tech] = F["YEAR_2020",tech]; # Generate F_new2010_2020
...
# Ca je vais le laisser commenter pour l'instant parce que c'est bizarre je dois en parler
...
# [Eq. XX] Limit the change for the PV and the wind
subject to limit_changes_renewable {p in PHASE, i in {"PV","WIND_ONSHORE","WIND_OFFSHORE"}} :
	F_new[p,i] <= limit_ren_changes*f_max["YEAR_2050",i];
...
subject to New_totalTransitionCost_calculation :
	TotalTransitionCost = C_tot_capex + C_tot_opex - sum {j in TECHNOLOGIES, p in PHASE} C_inv_return[j,p]; #avec le retour à la fin
	#TotalTransitionCost = C_tot_capex + C_tot_opex; # sans le retour à la fin

# [Eq. XX] True transition cost = TotalTransitionCost - C_inv 2020 - C_opex 2020
#subject to TrueTransitionCost_calculation :
#	TrueTransitionCost = TotalTransitionCost - sum {i in TECHNOLOGIES} C_inv ["YEAR_2020",i] - C_opex["YEAR_2020"];

# This equation takes into account the fact that technologies will still be available after 2050, so it is not bad if some technologies are installed at the end.
# [Eq. XX] Compute the return on investment of a technology that will be available after 2050.
subject to Cinv_return_end {j in TECHNOLOGIES, p in PHASE, y_start in PHASE_START[p], y_stop in PHASE_STOP[p]}: # investment return for the technology still existing at the end
	C_inv_return [j,p] = if years_done[j,p] = 100 then 0
					else (F_new[p,j] - sum {p2 in PHASE} (F_decom[p2,p,j])) * (1 - years_done[j,p]/lifetime["YEAR_2020",j]) * annualised_factor[p] * (c_inv[y_start,j]+c_inv[y_stop,j])/2;

# [Eq. XX] Compute capital expenditure for transition
subject to total_capex: # category: COST_calc
	#C_tot_capex = sum {i in TECHNOLOGIES} C_inv ["YEAR_2020",i] # 2020 investment
	#			 + sum{p in PHASE} C_inv_phase [p];# euros_2015
	C_tot_capex = sum {i in TECHNOLOGIES} C_inv ["YEAR_2020",i] + sum{p in PHASE} C_inv_phase [p];
