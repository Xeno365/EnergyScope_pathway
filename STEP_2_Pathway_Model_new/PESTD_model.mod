...
subject to use_all_the_waste {y in YEARS diff {"YEAR_2015","YEAR_2020"}} :
...
subject to phase_new_build {p in PHASE, y_start in PHASE_START[p], y_stop in PHASE_STOP[p], i in TECHNOLOGIES}:
	F[y_stop,i] = F[y_start,i] + F_new[p,i] - F_old [p,i] - sum {p2 in {PHASE union {"2010_2015"}}} F_decom[p,p2,i];
...
subject to delta_change_definition {p in PHASE diff {"2015_2020"}, y_start in PHASE_START[p], y_stop in PHASE_STOP[p], j in TECHNOLOGIES} :
...
subject to limit_changes_heat {p in PHASE diff {"2015_2020"}, y_start in PHASE_START[p], y_stop in PHASE_STOP[p]} :
...
subject to limit_changes_mob {p in PHASE diff {"2015_2020"}, y_start in PHASE_START[p], y_stop in PHASE_STOP[p]} :
...
subject to limit_changes_fright {p in PHASE diff {"2015_2020"}, y_start in PHASE_START[p], y_stop in PHASE_STOP[p]} :
...
subject to limit_changes_renewable {p in PHASE diff {"2015_2020"}, i in {"PV","WIND_ONSHORE","WIND_OFFSHORE"}} :
...
subject to Cinv_return_end {j in TECHNOLOGIES, p in PHASE, y_start in PHASE_START[p], y_stop in PHASE_STOP[p]}: # investment return for the technology still existing at the end
	C_inv_return [j,p] = if years_done[j,p] = 100 then 0
					else (F_new[p,j] - sum {p2 in PHASE} (F_decom[p2,p,j])) * (1 - years_done[j,p]/lifetime["YEAR_2015",j]) * annualised_factor[p] * (c_inv[y_start,j]+c_inv[y_stop,j])/2;
...
#subject to sameInvestmentPerPhase {p in PHASE diff {"2015_2020"}}:
